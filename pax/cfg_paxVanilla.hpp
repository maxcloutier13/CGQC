// --- paxVanilla ----------------------------------------------------------
// Group definitions when theres no assets loaded 

// *** Civilians ***************************************************************************************************************
cgqc_pax_civ = ["C_man_p_beggar_F", "C_man_1", "C_Man_casual_1_F", "C_Man_casual_2_F", "C_Man_casual_3_F", "C_Man_casual_4_v2_F", "C_Man_casual_5_v2_F", "C_Man_casual_6_v2_F", "C_Man_casual_7_F", "C_Man_casual_8_F", "C_Man_casual_9_F", "C_Man_casual_isntArt", "C_Man_smart_casual_1_F", "C_Man_smart_casual_2_F", "C_man_sport_1_F", "C_man_sport_2_F", "C_man_sport_3_F", "C_man_polo_1_F", "C_man_polo_2_F", "C_man_polo_3_F", "C_man_polo_4_F", "C_man_polo_5_F", "C_man_polo_6_F", "C_man_shorts_1_F", "C_man_1_1_F", "C_man_1_2_F", "C_man_1_3_F", "C_Man_ConstructionWorker_01_Black_F", "C_Man_ConstructionWorker_01_Blue_F", "C_Man_ConstructionWorker_01_Red_F", "C_Man_ConstructionWorker_01_Vrana_F", "C_Man_Fisherman_01_F", "C_journalist_F", "C_Man_Messenger_01_F", "C_Man_Paramedic_01_F", "C_man_shorts_2_F", "C_man_shorts_3_F", "C_man_shorts_4_F", "TCGM_F_C_Mini_Navy", "TCGM_F_C_Mini_Casual2", "TCGM_F_C_Mini_Casual3", "TCGM_F_C_Mini_Casual4", "TCGM_F_C_Mini_Casual5", "TCGM_F_C_Sport_5", "TCGM_F_C_Sport_4", "TCGM_F_C_Sport_3", "C_Man_UtilityWorker_01_F"];
cgqc_pax_civ_vips = ["C_Man_formal_1_F", "C_Man_formal_2_F", "TCGM_f_Thong_Hearts_C", "C_Man_formal_3_F", "C_Man_formal_4_F"];

// *** CSAT de marde ***************************************************************************************************************
cgqc_pax_opfor_vip = ["O_Officer_Parade_F", "O_Officer_Parade_Veteran_F"];
cgqc_pax_opfor_lead = ["O_Soldier_TL_F", "O_Soldier_SL_F", "O_officer_F"];
cgqc_pax_opfor_sniper = ["O_Sharpshooter_F", "O_soldier_M_F", "O_sniper_F", "O_A_soldier_M_F"];
cgqc_pax_opfor_soldier = ["O_Soldier_AR_F", "O_medic_F", "O_engineer_F", "O_Soldier_GL_F", "O_soldier_M_F", "O_Soldier_F", "O_Soldier_LAT_F", "O_soldier_UAV_F"];
//Vics
cgqc_pax_opfor_vic_tank = ["O_MBT_04_cannon_F", "O_MBT_04_command_F", "O_MBT_02_cannon_F"];
cgqc_pax_opfor_vic_apc = ["O_APC_Tracked_02_cannon_F", "O_APC_Wheeled_02_rcws_v2_F"];
cgqc_pax_opfor_vic_transport_squad = ["O_Truck_02_transport_F", "O_Truck_02_covered_F", "O_Truck_03_transport_F", "O_Truck_03_covered_F"];
cgqc_pax_opfor_vic_transport_team = ["O_MRAP_02_F", "O_LSV_02_unarmed_F"];
cgqc_pax_opfor_vic_transport_recon = ["O_Quadbike_01_F"];
cgqc_pax_opfor_vic_transport_fuel = ["O_Truck_02_fuel_F", "O_Truck_03_fuel_F"];
cgqc_pax_opfor_vic_transport_ammo = ["O_Truck_02_Ammo_F", "O_Truck_03_ammo_F"];
cgqc_pax_opfor_vic_transport_repair = ["O_Truck_02_box_F", "O_Truck_03_repair_F"];
cgqc_pax_opfor_vic_technical_mg = ["O_MRAP_02_hmg_F", "O_LSV_02_armed_F"];
cgqc_pax_opfor_vic_technical_gl = ["O_MRAP_02_gmg_F", "O_Soldier_F", "O_LSV_02_AT_F"];
//Statics
cgqc_pax_opfor_aa_light = ["O_static_AA_F"];
cgqc_pax_opfor_aa_heavy = ["O_APC_Tracked_02_AA_F"];
cgqc_pax_opfor_fixed_light = ["O_HMG_01_high_F", "O_Soldier_F", "O_HMG_01_F"];
cgqc_pax_opfor_fixed_heavy = ["O_static_AT_F", "O_Soldier_F", "O_GMG_01_F", "O_GMG_01_high_F"];
cgqc_pax_opfor_mortar = ["O_Mortar_01_F"];
cgqc_pax_opfor_arty = ["O_MBT_02_arty_F"];
//Air
cgqc_pax_opfor_radar = ["O_Radar_System_02_F"];
cgqc_pax_opfor_sam = ["O_SAM_System_04_F"];
cgqc_pax_opfor_plane_transport = ["uns_an2_transport"];
cgqc_pax_opfor_plane_fighter = ["O_Plane_Fighter_02_F", "O_Plane_Fighter_02_Stealth_F"];
cgqc_pax_opfor_plane_cas = ["O_Plane_CAS_02_dynamicLoadout_F"];
cgqc_pax_opfor_heli_cas_light = ["O_Heli_Light_02_dynamicLoadout_F"];
cgqc_pax_opfor_heli_cas_heavy = ["O_Heli_Attack_02_dynamicLoadout_F"];
cgqc_pax_opfor_heli_transport = ["O_Heli_Transport_04_covered_F"];
//Special shit
cgqc_pax_opfor_uav = ["O_UAV_01_F", "O_UAV_02_dynamicLoadout_F"];
cgqc_pax_opfor_ugv = ["O_UGV_01_rcws_F", "O_UGV_01_F"];
cgqc_pax_opfor_boat_light = ["O_Boat_Transport_01_F"];
cgqc_pax_opfor_boat_heavy = ["O_Boat_Armed_01_hmg_F"];
cgqc_pax_opfor_sub = ["O_SDV_01_F"];