class CfgGradPersistence {
    missionTag = "my_persistent_mission"; //String - The tag that everything in this mission will be saved under. Two missions with the same tag will overwrite each other when saving, even on different terrains. One mission can load the data of a different mission, if they are both saved under the same tag.
    loadOnMissionStart = 1;  //0/1 - Toggles automatic loading on start of mission.
    ...
    saveVehicles = 1; //Vehicles, static weapons, certain objects.
    saveContainers = 1; //Containers such as ammo boxes.
    saveStatics = 1; //Static objects such as houses, walls, trees, etc. excluding those created with grad-fortifications.
