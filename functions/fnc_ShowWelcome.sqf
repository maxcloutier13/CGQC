...
if (cgqc_setting_show_welcome) then {
	LOG(" welcome - is true: Showing");
	if(cgqc_player_known) then {
		LOG(" welcome - player is known");
	...
	LOG(" welcome - waiting for intro to finish");
	waitUntil {cgqc_intro_done};
	LOG(" welcome - intro finished");
	...
	if (cgqc_player_known) then {
		LOG(" welcome - Known player popup on the side");
		...
		LOG(" welcome - Unknown player popup fullscreen");
