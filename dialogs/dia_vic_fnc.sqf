...
{	
	...
	createDialog "RscCGQCVehDialog";
	["Car"] call CGQC_gui_LoadVeh;
...
dia_vic_guiCreate = 
{
	...
	_emptyPos = getPosASL player findEmptyPosition [10,50,_classVeh];
	if (count _emptyPos == 0) then { hint "Vehicle cannot be spawned here"; }
	else
	{	
		cgqc_spawn_vic = createVehicle [_classVeh, _emptyPos, [], 0,""];
		cgqc_spawn = true;
	};
	closeDialog 1601;
};
