...
case "swat_assaulter":{
	...
	["tl"] spawn CGQC_fnc_getRadioPresets;
...
case "swat_fto":{
	LOG("[switchRole] FTO calling swat_others");
	[player, "fto", "none"] call CGQC_ldt_swat_others;
	LOG("[switchRole] FTO switching perks");
	["hq"] spawn CGQC_fnc_switchPerks;
	LOG("[switchRole] FTO switching radios");
	["tl"] spawn CGQC_fnc_getRadioPresets;
	LOG("[switchRole] FTO setting variables");
	cgqc_player_role = "Field Training Officer";
	player setVariable ["cgqc_player_roleType", "HQ", true];
	cgqc_player_role_optic = 6;
	LOG("[switchRole] FTO switch done");
};